...
typedef struct Vertex{ 										// 간선과 정점의 정보를 저장하기 위한 구조체
	int data;
	struct Vertex *link;
} Vertex;
...
/* for queue */
#define MAX_QUEUE_SIZE 20 									// 큐의 최대 크기를 20으로 설정
...
int VisitVertex(int visitV);							    // 방문 정보를 저장하는 함수
...
int main()
{
	...
	do{
		...
		case 'z': case 'Z':
			...
			break;
		case 'q': case 'Q':
			freeGraph(graph);								 // 프로그램 끝날때 메모리 해제
			break;
		case 'v': case 'V':
			printf("Your Num = ");
			scanf("%d", &v);
			insertVertex(graph, v);							 // 정점을 생성하기 위해 함수 호출
			break;
		case 'd': case 'D':
			printf("Your Num = ");
			scanf("%d", &v);
			initVist();										 // DepthFS하기 전에 방문정보 초기화
			DepthFS(graph, v);								 // 정점에 대한 DFS 실행
			break;
		case 'p': case 'P':
		...
		case 'e': case 'E':
			printf("Your Num = ");
			scanf("%d %d", &u, &v);
			insertEdge(graph, u, v);						 // 두 정점을 입력받아 Edge 삽입
			insertEdge(graph, v, u); 						 // 무방향 그래프이기 때문에 양쪽에 삽입되어야 해서 반대로 한번 더 실행
			break;
		case 'b': case 'B':
			printf("Your Num = ");
			scanf("%d", &v);
...
int freeGraph(Graph *G)
{
	...
	for(int i = 0; i < MAX_VERTEX; i++){
		...
		if(n != NULL) { 									// n이 NULL이 아니면
			while(n != NULL){ 								// n이 NULL이 될 때까지
				trail = n; 									// trail을 n으로 초기화하고
				n = n->link;								// n은 n->link가 되게 함
...
int insertVertex(Graph *G, int v)
{
	...
	if (v > MAX_VERTEX){
		printf("[%d] is out of range!!\n"); 				// 정점의 최대 개수를 넘어갈 경우 범위를 넘겼다는 내용 출력
		return 0;
	}

	else if(G[v].Head != NULL){
		printf("The Vertex of [%d] already exists!!\n", v); // 같은 번호의 정점이 있을경우 이미 있다는 내용 출력
		return 0;
	}

	else{
		G[v].Head = new; 									//위에 두 경우를 제외한 나머지는 G[v].Head를 new로 초기화
		return 0;
	}
}
...
int insertEdge(Graph *G, int u, int v)
{
	...
	if(CheckV(G, u) != 1 || CheckV(G, v) != 1) { 			// 두 정점이 생성되어 있는지 확인
	...
	while(n != NULL) { 										// n이 NULL이 되기 전까지
		if(n->data > v) {
			...
			if(n == G[u].Head->link) {						// 첫 노드에 삽입해야할 경우
				new->link = G[u].Head->link; 				// 새로운 노드의 link를 head의 link로 변경
				G[u].Head->link = new; 						// head의 link가 새로운 노드를 가리키게 함
			} 
			else {											// 중간이거나 마지막인 경우 
				new->link = trail->link; 					// 새로운 노드의 link를 trail->link로 변경
...
{
	if(v > MAX_VERTEX) return 0; 							// 정점의 최대개수가 넘어갈 경우 0 리턴

	else if(G[v].Head == NULL) return 0; 					// 정점이 생성되지 않았으면 0 리턴

	else return 1; 											// 정상적으로 생성되어있는 경우 1 리턴
}

int VisitVertex(int visitV)
{
	if(visitInfo[visitV] == 0){ 							// 방문한 적이 없으면
		visitInfo[visitV] = 1; 								// 방문했다는 의미로 해당 번호의 배열에 1로 변경
		return 0; 											// 방문한 적이 없었으므로 0 반환
	}
...
void printGraph(Graph *G)
{
	...
	for(int i = 0; i < MAX_VERTEX; i++){ 					// i가 0부터 MAX_VERTEX - 1 까지
